...
typedef enum mjs_err {
  ...
  MJS_ERRS_CNT,
  MJS_TERMINATE			//CUSTOM R.Piontik terminate process
} mjs_err_t;
...
/* ******************************************************************
 * 							CUSTOM DEFINE R.Piontik
 * ******************************************************************/

#ifndef CS_STDERROR
	FILE * vm_stderr __attribute__((weak)) = NULL;				//stderr or script output
	#define CS_STDERROR						vm_stderr
#endif

#ifndef CS_STDOUT
	FILE * vm_stdout __attribute__((weak)) = NULL;				//stdout stream
	#define CS_STDOUT						vm_stdout
#endif

//Return this object
mjs_val_t mjs_this(struct mjs *mjs);
...
enum mjs_debugger_action {
	MJS_DBGR_ACT_RUN,
	MJS_DBGR_ACT_STEP_INTO,
	MJS_DBGR_ACT_STEP_OVER,
	MJS_DBGR_ACT_STEP_OUT,		//Step out current function
	MJS_DBGR_ACT_TERMINATE	 	//Full disable debugger for this process
};

//CUSTOM R.Piontik
typedef enum mjs_debugger_action (mjs_debugger_request_t)(struct mjs *mjs, const char *filename, int line_no);
typedef enum mjs_debugger_action (mjs_debugger_handle_t)(struct mjs *mjs, const char *filename, int line_no);
typedef enum mjs_debugger_action (mjs_debugger_on_error_t)(struct mjs *mjs, const char *filename, int line_no);
...
/* CUSTOM R.Piontik
 * Set readonly market on propery
 */
void mjs_set_protected(struct mjs *mjs, mjs_val_t obj, const char *name, size_t len, bool is_protected);

/* CUSTOM R.Piontik
 * Apply debugger listeners
 */
void mjs_apply_debugger(struct mjs *mjs, mjs_debugger_request_t * requester, mjs_debugger_handle_t * handler,
