...
/*
 * Set file to write logs into. If `NULL`, logs go to `CS_STDERROR`.
 */
...
struct mjs {
  ...
  //CUSTOM R.Piontik
  bool dbgr_enabled;						//If true - debugger is enabled
  mjs_debugger_request_t *dbgr_requester;	//"debugger" command listener
  mjs_debugger_handle_t *dbgr_handler;		//Debugger actions handler
  mjs_debugger_on_error_t *dbgr_on_error; 	//Error listener
  enum mjs_debugger_action dbgr_action; 	//Current debugger action
  int dbgr_current_line;					//Current line executing
  int dbgr_current_stack_pos;				//Current position in stack
  //CUSTOM
 ...
 */
mjs_val_t mjs_legit_pointer_to_value(void *p);
...
int cs_log_print_prefix(enum cs_log_level level, const char *func,
  ...
  prefix[20] = '\0';
  if (cs_log_file == NULL) cs_log_file = CS_STDERROR;
...
static void mjs_print(struct mjs *mjs) {
  ...
  for (i = 0; i < num_args; i++) {
    mjs_fprintf(mjs_arg(mjs, i), mjs, CS_STDOUT);	//CUSTOM stdout >> CS_STDOUT
    putchar(' ');
  }
  putchar('\n');
  fflush(CS_STDOUT); //CUSTOM
...
struct mjs *mjs_create(mjs_val_t * global) {
  ...
  mjs->bcode_len = 0;
  //CUSTOM R.Piontik
  mjs->dbgr_action = MJS_DBGR_ACT_RUN;
  mjs->dbgr_current_line = -1;
  mjs->dbgr_current_stack_pos = 0;
  //CUSTOM
...
void mjs_clear_error(struct mjs *mjs) {
	mjs->error = MJS_OK;
	if(mjs->error_msg)
		free(mjs->error_msg);
	mjs->error_msg = NULL;
...
enum mjs_debugger_action mjs_get_debugger_status(struct mjs *mjs) {
	return mjs->dbgr_action;
}

void mjs_apply_debugger(
		struct mjs *mjs,
		mjs_debugger_request_t * requester,
		mjs_debugger_handle_t * handler,
		mjs_debugger_on_error_t * error_listener) {
	mjs->dbgr_requester = requester;
	mjs->dbgr_handler = handler;
	mjs->dbgr_on_error = error_listener;
}

void mjs_debugger_enable(struct mjs *mjs, bool mode) {
	mjs->dbgr_enabled = mode;
}

void mjs_debugger_do_handle(struct mjs *mjs, int offset) {
	int line_no = mjs_get_lineno_by_offset(mjs, offset);

	if(mjs->dbgr_current_line == line_no)
		return;

	switch(mjs->dbgr_action) {
		case MJS_DBGR_ACT_STEP_OVER:
			if(mjs->call_stack.len > mjs->dbgr_current_stack_pos)
				return;
			break;
		case MJS_DBGR_ACT_STEP_OUT:
			if((mjs->call_stack.len > 0) && (mjs->dbgr_current_stack_pos <= mjs->call_stack.len))
				return;
			break;
		case MJS_DBGR_ACT_TERMINATE:
			mjs_set_errorf(mjs, MJS_TERMINATE, "Debuging is terminated");
			return;
		default:
			break;
	}
	...
	const char *filename = mjs_get_bcode_filename_by_offset(mjs, offset);
	mjs->dbgr_current_line = line_no;
	mjs->dbgr_current_stack_pos = mjs->call_stack.len;
...
MJS_PRIVATE void mjs_debugger_request(struct mjs *mjs, int offset) {
    if(mjs->dbgr_enabled && mjs->dbgr_requester) {
        const char *filename = mjs_get_bcode_filename_by_offset(mjs, offset);
        int line_no = mjs_get_lineno_by_offset(mjs, offset);
        mjs->dbgr_action = mjs->dbgr_requester(mjs, filename, line_no);
    }
}
...
MJS_PRIVATE void mjs_debugger_onerror(struct mjs *mjs, int offset) {
    if(mjs->dbgr_enabled && mjs->dbgr_on_error) {
        const char *filename = mjs_get_bcode_filename_by_offset(mjs, offset);
        int line_no = mjs_get_lineno_by_offset(mjs, offset);
        mjs->dbgr_action = mjs->dbgr_on_error(mjs, filename, line_no);
    }
}
...
MJS_PRIVATE mjs_err_t mjs_execute(struct mjs *mjs, size_t off, mjs_val_t *res) {
  ...
  for (i = off; i < bp.data.len; i++) {
	...
	//CUSTOM R.Piotnik
	mjs_debugger_handle(mjs, bp.start_idx + i - 1);
	//CUSTOM
    ...
    switch (opcode) {
      ...
      //CUSTOM R.Piontik
      case OP_DEBUGGER:
    	  mjs_debugger_request(mjs, bp.start_idx + i - 1);
    	  i++;
        break;
      //CUSTOM
      case OP_EXIT:
    ...
    if (mjs->error != MJS_OK) {
    	...
    	//CUSTOM R.Piontik
    	mjs_debugger_onerror(mjs, bp.start_idx + i - 1);
    	//CUSTOM
...
MJS_PRIVATE mjs_err_t mjs_parse_ffi_signature(struct mjs *mjs, const char *s,
  ...
  fflush(CS_STDOUT); //CUSTOM stdout >> CS_STDOUT
...
static union ffi_cb_data_val ffi_cb_impl_generic(void *param,
  ...
  if (err != MJS_OK) {
     ...
     */
    mjs_print_error(mjs, CS_STDERROR, "MJS callback error",
                    1 /* print_stack_trace */);
...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc && argv[i][0] == '-' && err == MJS_OK; i++) {
    if (strcmp(argv[i], "-l") == 0 && i + 1 < argc) {
      ...
      fprintf(CS_STDERROR, "Unknown flag: [%s]\n", argv[i]);
      return EXIT_FAILURE;
    }
  }
  ...
  if (err == MJS_OK) {
    mjs_fprintf(res, mjs, CS_STDOUT); //CUSTOM stdout >> CS_STDOUT
    putchar('\n');
  } else {
    mjs_print_error(mjs, CS_STDOUT, NULL, 1 /* print_stack_trace */); //CUSTOM stdout >> CS_STDOUT
  }
  mjs_destroy(mjs);
...
// CUSTOM R.Piontik
void mjs_set_protected(struct mjs *mjs, mjs_val_t obj, const char *name, size_t len, bool is_protected) {
	struct mjs_property *p = mjs_get_own_property(mjs, obj, name, len);
	if(p)
		p->readonly = is_protected;
}
